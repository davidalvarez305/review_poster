...
func insertCategories(products []types.AmazonSearchResultsPage, parent_group, keyword string) ([]models.Category, error) {
	var categories []models.Category
	var parentGroups []models.ParentGroup
	var categoryGroup models.CategoryGroup
	var c []string
	var parentGroup models.ParentGroup
	var createdCategoryGroup models.CategoryGroup

	req := database.DB.Find(&parentGroups)

	if req.Error != nil {
		fmt.Println("Error while trying to insert parent groups.")
		return nil, req.Error
	}

	for i := 0; i < len(parentGroups); i++ {
		if parentGroups[i].Name == parent_group {
			parentGroup = parentGroups[i]
		}
	}

	if !(parentGroup.ID > 0) {
		return categories, fmt.Errorf("could not find parent group")
	}
	...
	fmt.Printf("Inserting %s category group!", categoryGroup.Name)
	ins := database.DB.Clauses(clause.OnConflict{DoNothing: true}).Save(categoryGroup)

	if ins.Error != nil {
		fmt.Println("Error while trying to insert category group.")
		return nil, ins.Error
	}

	cg := database.DB.Where("name = ?", categoryGroup.Name).First(&createdCategoryGroup)
...
func InsertReviewPosts(parent_group, keyword string, products []types.AmazonSearchResultsPage, dictionary []types.Dictionary, sentences []types.DynamicContent) error {
	var posts []models.ReviewPost

	_, err := insertProducts(products)

	if err != nil {
		return err
	}

	c, err := insertCategories(products, parent_group, keyword)
	...
	for i := 0; i < len(products); i++ {
		...
		var categoryId int
		for _, cat := range c {
			if makeTitle.String(products[i].Category) == cat.Name {
				categoryId = cat.ID
			}
		}
